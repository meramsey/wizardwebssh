...
class TestAppBasic(TestAppBase):
    ...
    def setUpClass(cls):
        print("=" * 20)
        t = threading.Thread(target=run_ssh_server, args=(cls.sshserver_port, cls.running), daemon=True)
        # t.setDaemon(True)
        t.start()
    ...
    def test_app_auth_with_valid_pubkey_by_multipart_form(self):
        ...
        content_type, body = encode_multipart_formdata(self.body_dict.items(), files)
    ...
    def test_app_with_user_pass2fa_with_correct_passwords(self):
        self.body_dict.update(username="pass2fa", password="password", totp="passcode")
        response = yield self.async_post("/", self.body_dict)
        self.assertEqual(response.code, 200)
        data = json.loads(to_str(response.body))
        self.assert_status_none(data)
    ...
    def test_app_with_user_pass2fa_with_wrong_pkey_correct_passwords(self):
        url = self.get_url("/")
        privatekey = read_file(make_tests_data_path("user_rsa_key"))
        self.body_dict.update(username="pass2fa", password="password", privatekey=privatekey, totp="passcode")
        response = yield self.async_post(url, self.body_dict)
        data = json.loads(to_str(response.body))
        self.assert_status_none(data)
    ...
    def test_app_with_user_pkey2fa_with_correct_passwords(self):
        url = self.get_url("/")
        privatekey = read_file(make_tests_data_path("user_rsa_key"))
        self.body_dict.update(username="pkey2fa", password="password", privatekey=privatekey, totp="passcode")
        response = yield self.async_post(url, self.body_dict)
        data = json.loads(to_str(response.body))
        self.assert_status_none(data)
    ...
    def test_app_with_user_pkey2fa_with_wrong_password(self):
        url = self.get_url("/")
        privatekey = read_file(make_tests_data_path("user_rsa_key"))
        self.body_dict.update(username="pkey2fa", password="wrongpassword", privatekey=privatekey, totp="passcode")
        response = yield self.async_post(url, self.body_dict)
        data = json.loads(to_str(response.body))
    ...
    def test_app_with_user_pkey2fa_with_wrong_passcode(self):
        url = self.get_url("/")
        privatekey = read_file(make_tests_data_path("user_rsa_key"))
        self.body_dict.update(username="pkey2fa", password="password", privatekey=privatekey, totp="wrongpasscode")
        response = yield self.async_post(url, self.body_dict)
        data = json.loads(to_str(response.body))
        self.assert_status_in("Authentication failed", data)
    ...
    def test_app_with_user_pkey2fa_with_empty_passcode(self):
        url = self.get_url("/")
        privatekey = read_file(make_tests_data_path("user_rsa_key"))
        self.body_dict.update(username="pkey2fa", password="password", privatekey=privatekey, totp="")
        response = yield self.async_post(url, self.body_dict)
        data = json.loads(to_str(response.body))
...
class OtherTestBase(TestAppBase):
    ...
    origin = "same"
    encodings: List[str] = []
    body = {"hostname": "127.0.0.1", "port": "", "username": "robey", "password": "foo", "_xsrf": "yummy"}
    ...
    def setUp(self):
        ...
        t = threading.Thread(
            target=run_ssh_server, args=(self.sshserver_port, self.running, self.encodings), daemon=True
        )
        t.start()
...
class TestAppInDebugMode(OtherTestBase):  # type: ignore
...
class TestAppWithRejectPolicy(OtherTestBase):  # type: ignore

    policy = "reject"  # type: ignore
...
class TestAppWithBadHostKey(OtherTestBase):  # type: ignore

    policy = random.choice(["warning", "autoadd", "reject"])  # type: ignore
...
class TestAppNotFoundHandler(OtherTestBase):
    ...
    def test_with_not_found_get_request(self):
        ...
        self.assertEqual(response.headers["Server"], self.custom_headers["Server"])
    ...
    def test_with_not_found_put_request(self):
        response = self.fetch_request("/pathnotfound", method="PUT", body=self.body)
        self.assertEqual(response.code, 404)
        self.assertEqual(response.headers["Server"], self.custom_headers["Server"])
        self.assertIn(b"404: Not Found", response.body)
...
class TestAppWithCrossOriginOperation(OtherTestBase):
    ...
    def test_app_with_wrong_header_origin(self):
        ...
        self.assert_status_equal(
            ...
            json.loads(to_str(response.body)),
        )  # noqa
